/**
 * §2 eleven-step inventory — fails if step order or missing endpoints break.
...
const STEP_FILES: Array<{ step: number; path: string; mustMatch: RegExp }> = [
  ...
  { step: 8, path: "../enrol-requirements/index.ts", mustMatch: /step:\s*8|enrolment_step.*8/ },
  { step: 9, path: "../enrol-baseline/index.ts", mustMatch: /step:\s*9|enrolment_step.*9/ },
  { step: 10, path: "../enrol-complete/index.ts", mustMatch: /step:\s*10|enrolment_step.*10/ },
  { step: 11, path: "../enrol-handoff/index.ts", mustMatch: /step:\s*11|enrolment_step.*11/ },
];

Deno.test("all eleven §2 steps have endpoints in order", async () => {
  ...
  // Strict ascending order of step numbers in inventory
