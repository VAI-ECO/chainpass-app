...
DROP POLICY IF EXISTS "Admins can view digest settings" ON public.email_digest_settings;
CREATE POLICY "Admins can view digest settings"
ON public.email_digest_settings
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can update digest settings" ON public.email_digest_settings;
CREATE POLICY "Admins can update digest settings"
ON public.email_digest_settings
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can insert digest settings" ON public.email_digest_settings;
...
DROP POLICY IF EXISTS "Admins can view digest recipients" ON public.email_digest_recipients;
CREATE POLICY "Admins can view digest recipients"
ON public.email_digest_recipients
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can manage digest recipients" ON public.email_digest_recipients;
...
DROP POLICY IF EXISTS "Admins can view digest history" ON public.email_digest_history;
CREATE POLICY "Admins can view digest history"
ON public.email_digest_history
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage digest history" ON public.email_digest_history;
