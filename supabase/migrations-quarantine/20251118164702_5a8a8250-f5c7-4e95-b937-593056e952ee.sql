...
DROP POLICY IF EXISTS "Users can view their own signed contracts" ON public.signed_contracts;
CREATE POLICY "Users can view their own signed contracts"
ON public.signed_contracts
  FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Service role can insert signed contracts" ON public.signed_contracts;
CREATE POLICY "Service role can insert signed contracts"
ON public.signed_contracts
  FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins can view all signed contracts" ON public.signed_contracts;
...
DROP POLICY IF EXISTS "Service role can insert signature attempts" ON public.signature_attempts;
CREATE POLICY "Service role can insert signature attempts"
ON public.signature_attempts
  FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins can view all signature attempts" ON public.signature_attempts;
