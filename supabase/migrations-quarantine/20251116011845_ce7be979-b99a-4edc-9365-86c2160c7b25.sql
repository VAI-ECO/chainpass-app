...
DROP POLICY IF EXISTS "Business partners can view their own usage logs" ON api_usage_logs;
...
DROP POLICY IF EXISTS "Service role can insert usage logs" ON api_usage_logs;
...
DROP POLICY IF EXISTS "Business partners can view their own data" ON business_partners;
CREATE POLICY "Business partners can view their own data"
ON business_partners
FOR SELECT
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Business partners can update their own data" ON business_partners;
