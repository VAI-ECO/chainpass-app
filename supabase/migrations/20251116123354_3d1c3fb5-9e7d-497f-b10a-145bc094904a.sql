...
DROP POLICY IF EXISTS "Users can view their own test scenarios" ON public.sandbox_test_scenarios;
CREATE POLICY "Users can view their own test scenarios"
ON public.sandbox_test_scenarios FOR SELECT
  USING (auth.uid() = user_id OR is_public = true);

DROP POLICY IF EXISTS "Users can create their own test scenarios" ON public.sandbox_test_scenarios;
CREATE POLICY "Users can create their own test scenarios"
ON public.sandbox_test_scenarios FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own test scenarios" ON public.sandbox_test_scenarios;
CREATE POLICY "Users can update their own test scenarios"
ON public.sandbox_test_scenarios FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own test scenarios" ON public.sandbox_test_scenarios;
CREATE POLICY "Users can delete their own test scenarios"
ON public.sandbox_test_scenarios FOR DELETE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all test scenarios" ON public.sandbox_test_scenarios;
...
DROP POLICY IF EXISTS "Admins can view webhook replay history" ON public.webhook_replay_history;
CREATE POLICY "Admins can view webhook replay history"
ON public.webhook_replay_history FOR SELECT
  USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Business partners can view their webhook replays" ON public.webhook_replay_history;
...
DROP POLICY IF EXISTS "Service role can manage webhook replay history" ON public.webhook_replay_history;
