...
DROP POLICY IF EXISTS "Admins can update retention policies" ON public.retention_policies;
CREATE POLICY "Admins can update retention policies"
ON public.retention_policies
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can insert retention policies" ON public.retention_policies;
...
DROP POLICY IF EXISTS "Admins can view archived logs" ON public.archived_activity_logs;
CREATE POLICY "Admins can view archived logs"
ON public.archived_activity_logs
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage archived logs" ON public.archived_activity_logs;
