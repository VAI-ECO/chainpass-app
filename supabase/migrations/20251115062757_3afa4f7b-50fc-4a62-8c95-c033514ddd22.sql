...
DROP POLICY IF EXISTS "Admins can view all performance scores" ON public.admin_performance_scores;
CREATE POLICY "Admins can view all performance scores"
ON public.admin_performance_scores
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage performance scores" ON public.admin_performance_scores;
...
DROP POLICY IF EXISTS "Admins can view badges" ON public.admin_badges;
CREATE POLICY "Admins can view badges"
ON public.admin_badges
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage badges" ON public.admin_badges;
...
DROP POLICY IF EXISTS "Admins can view all earned badges" ON public.admin_earned_badges;
CREATE POLICY "Admins can view all earned badges"
ON public.admin_earned_badges
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage earned badges" ON public.admin_earned_badges;
