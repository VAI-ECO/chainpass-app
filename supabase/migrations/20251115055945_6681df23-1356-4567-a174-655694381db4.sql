...
DROP POLICY IF EXISTS "Admins can view anomaly settings" ON public.anomaly_detection_settings;
CREATE POLICY "Admins can view anomaly settings"
ON public.anomaly_detection_settings
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can update anomaly settings" ON public.anomaly_detection_settings;
CREATE POLICY "Admins can update anomaly settings"
ON public.anomaly_detection_settings
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can insert anomaly settings" ON public.anomaly_detection_settings;
...
DROP POLICY IF EXISTS "Admins can view detected anomalies" ON public.detected_anomalies;
CREATE POLICY "Admins can view detected anomalies"
ON public.detected_anomalies
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can update detected anomalies" ON public.detected_anomalies;
CREATE POLICY "Admins can update detected anomalies"
ON public.detected_anomalies
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Service role can manage detected anomalies" ON public.detected_anomalies;
