...
export default function FinalVerification() {
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      // Try to get LEO status from database first (if VAI exists)
      const vaiNumber = sessionManager.getVaiCode();
      if (vaiNumber) {
        ...
        if (vaiData?.is_leo !== undefined) {
          setIsLEO(vaiData.is_leo);
        } else {
        ...
        // Fallback to sessionStorage if no VAI
        const leoStatus = sessionStorage.getItem('userType') === 'leo';
        setIsLEO(leoStatus);
      }
